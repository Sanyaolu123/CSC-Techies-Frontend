...
import type { Response, Timetable } from "@/interfaces"
import { fetchTimeTable } from "@/services"
...
function expandSessionsToEvents(data: Timetable[] | null): MyEvent[] {
  ...
  data?.forEach((item) => {
    ...
    while (current <= end) {
      ...
      item.sessions.forEach((session) => {
        if (day === dayOfWeekMap[session.dayOfWeek]) {
...
export default function TimetablePage() {
  ...
  useEffect(() => {
    const load = async () => {
      const timetable: Response<Timetable[]> = await fetchTimeTable()
